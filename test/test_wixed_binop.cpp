...
TEST(wixed_binop, test_wixed_addition) {
    auto a = WixedForm(AffineForm(Winterval(2, 3)), Winterval(-1, 2));
    auto b = WixedForm(AffineForm(Winterval(1, 4)), Winterval(0, 3));
...
TEST(wixed_binop, test_wixed_division) {
    auto a = WixedForm(AffineForm(Winterval(2, 3)), Winterval(-1, 2));
    auto b = WixedForm(AffineForm(Winterval(-1, 4)), Winterval(0, 2));
    auto result = a / b;
